...
& E\left(\hat{f}_h(x)\right)^2  \ - \ E\left(\hat{f}_h(x)\right)^2  \ - \ E\left(\hat{f}_h(x)\right)\cdot E\l
eft( f(x)\right) \ + \
...
```{remark include=FALSE}
<!-- NO LE ENCUENTRO SENTIDO A ESTE PÁRRAFO -->
...
##   add_model(modelo_svm %>% set_args(cost = tune())) %>%
##   add_formula(Purchase ~ .)

## set.seed(1234)
## Caravan_fold <- vfold_cv(Caravan.training, strata = Purchase, v = 10)

## param_grid <- grid_regular(cost(), levels = 10)

## tune_res <- tune_grid(
##   svm_lineal_wf,
##   resamples = Caravan_fold,
##   grid = param_grid
## )
